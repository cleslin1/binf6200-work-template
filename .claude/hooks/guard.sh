...
# Rule 4: file edits stay inside the project directory. Three places outside it are Claude Code's
# own working files, written with the same tools, so they are allowed:
#   ~/.claude/plans/*.md                  the plan Plan Mode saves before you approve it
#   ~/.claude/projects/*/memory/*.md      notes Claude Code keeps about a project between sessions
#   the session's scratchpad directory    a temporary folder Claude Code is given for working files
# A path with ".." in it is never allowed, so none of these can be used to reach anything else.
proj="${CLAUDE_PROJECT_DIR:-$PWD}"
scratch=$(json_field scratchpad_dir) || unreadable
allowed_outside() {
  [[ "$path" == *"/../"* || "$path" == *"/.." ]] && return 1
  [[ "$path" == "$HOME/.claude/plans/"*.md ]] && return 0
  [[ "$path" == "$HOME/.claude/projects/"*"/memory/"*.md ]] && return 0
  [[ -n "$scratch" && "$path" == "$scratch/"* ]] && return 0
  return 1
}
if [[ -n "$path" && "$path" != "$proj"* ]] && ! allowed_outside; then
